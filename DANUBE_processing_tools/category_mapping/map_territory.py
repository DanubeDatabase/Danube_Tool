...
def define_territory(df, DANUBE_LAYERS, PATH_DANUBE_TABLES_FOLDER):
    ...
    df["territory_comm"] = np.where(pd.isna(df['period_danube']), np.nan,
                                    np.where(df['period_danube'] == 'P1', ter_com_p1,
                                             ter_com_p2))

    nan_part = df[pd.isna(df['period_danube'])][["period_danube", "territory_comm"]]
    print_log("\nnan_part", nan_part.head(), "\n")
    print_log(df[["period_danube", "territory_comm"]].tail())
...
def main_cm_territory(df, DANUBE_LAYERS, PATH_DANUBE_TABLES_FOLDER):
    print_log("*" * 100)
