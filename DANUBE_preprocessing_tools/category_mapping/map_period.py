...
def year_option1(df):
    ...
    df['year_option1'] = df.apply(
        lambda row: int(row['topo_DATE_APP'][:4]) if pd.notnull(row['topo_DATE_APP']) else np.nan, axis=1)
...
def create_period(df):
    ...
    df['period_quality'] = df.apply(lambda row: 'A' if pd.notnull(row['year_option1']) else
